...
import data
def convert_single_example(max_seq_length,tokenizer,text_a):
...
class Example(object):
  ...
  def __init__(self, article, abstract_sentences, vocab, hps,tokenizer):
    ...
    art_ids,art_mask,art_seg,article_words=convert_single_example(hps.max_enc_steps,tokenizer,article)
    #self.enc_len = len(article_words) # store the length after truncation but before padding
    self.enc_len=len(art_ids)
    #self.enc_input = [vocab.word2id(w) for w in article_words] # list of word ids; OOVs are represented by the id for UNK token
    self.enc_input=art_ids
    # Process the abstract
    abstract = ' '.join(abstract_sentences) # string
    #abstract_words = abstract.split() # list of strings
    #abs_ids = [vocab.word2id(w) for w in abstract_words] # list of word ids; OOVs are represented by the id for UNK token
    abs_ids,abs_mask,abs_seg,abstract_words=convert_single_example(hps.max_seq_length,tokenizer,abstract)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    self.enc_segment_id=np.zeros((hps.batch_size,max_enc_seq_len),dtype=np.int32)
  ...
  def init_decoder_seq(self, example_list, hps):
    ...
    self.dec_segment_id = np.zeros((hps.batch_size, hps.max_dec_steps), dtype=np.int32)
    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
...
class Batcher(object):
  ...
  def __init__(self, data_path, vocab, hps, single_pass,tokenizer):
  ...
  def fill_example_queue(self):
    ...
    while True:
      ...
      example = Example(article, abstract_sentences, self._vocab, self._hps,self.tokenizer) # Process into an Example.
