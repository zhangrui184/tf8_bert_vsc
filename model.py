...
class SummarizationModel(object):
  ...
  def __init__(self, hps, vocab,bert_path):
    self._hps = hps
    self._vocab = vocab
  ...
  def _add_placeholders(self):
    ...
    self._enc_segment_id=tf.placeholder(tf.int32,[hps.batch_size,None],name='enc_segment_id')
    ...
    self._dec_segment_id = tf.placeholder(tf.int32, [hps.batch_size, None], name='dec_segment_id')
  ...
  def _make_feed_dict(self, batch, just_enc=False):
    ...
    if not just_enc:
      ...
      feed_dict[self._dec_segment_id] = batch.dec_segment_id
    return feed_dict
  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      bert_config=modeling.BertConfig.from_json_file(os.path.join(self.bert_path,"bert_config.json"))
      with tf.variable_scope("enc_bert"):
        self.enc_model=modeling.BertModel(
          config=bert_config,
          is_training=hps.mode,
          input_ids=self._enc_batch,
          input_mask=self._enc_padding_mask,
          token_type_ids=self._enc_segment_id,
          use_one_hot_embeddings=False
        )
      with tf.variable_scope("dec_bert"):
        self.dec_model=modeling.BertModel(
          config=bert_config,
          is_training=hps.mode,
          input_ids=self._dec_batch,
          input_mask=self._dec_padding_mask,
          token_type_ids=self._dec_segment_id,
          use_one_hot_embeddings=False
        )
      init_ckpt=os.path.join(self.bert_path,"bert_model.ckpt")
      enc_tvars=tf.trainable_variables(scope="enc_bert")
      dec_tvars=tf.trainable_variables(scope="dec_bert")
      (assignment_map,_)=modeling.get_assignment_map_from_checkpoint(enc_tvars,init_ckpt)
      tf.train.init_from_checkpoint(init_ckpt,assignment_map)
      (assignment_map,_)=modeling.get_assignment_map_from_checkpoint(dec_tvars,init_ckpt)
      tf.train.init_from_checkpoint(init_ckpt, assignment_map)
     # with tf.variable_scope('embedding'):
      ...
      emb_enc_inputs=self.enc_model.get_sequence_output()
      # Add the encoder.
      ...
      with tf.variable_scope('decoder'):
        emb_dec_inputs=self.dec_model.get_sequence_output()
        emb_dec_inputs=[x for x in tf.unstack(emb_dec_inputs,axis=1)]
